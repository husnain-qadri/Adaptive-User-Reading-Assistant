...
from backend.services.paper_parser import parse_pdf
from backend.config import MAX_PDF_DOWNLOAD_SIZE_MB, RESOLVE_WORKERS, S2_MAX_RPS
...
# ---------------------------------------------------------------------------
# S2 token-bucket rate limiter (A2)
# ---------------------------------------------------------------------------
...
def extract_references():
    ...
    # --- A3: batch-resolve references with known DOIs/arXiv IDs first ---
    ...
    # --- A1: concurrent resolution for remaining references ---
    ...
    # Merge batch results with individually resolved ones
...
def _get_citation_strings(doc: dict[str, Any], full_text: str) -> list[str]:
    """Return citation strings, with PaperMage/regex cross-check (C5)."""
...
# ---------------------------------------------------------------------------
# Helpers — citation extraction from raw text  (C1–C4)
# ---------------------------------------------------------------------------
...
def _extract_references_from_text(full_text: str) -> list[str]:
    # C1: prefer the *last* match (bibliography is near the end)
    ...
    # C4: truncate at the next major heading after the bibliography
    end_match = _POST_REF_HEADER_RE.search(ref_section)
    if end_match:
        ref_section = ref_section[:end_match.start()]

    # C2: allow start-of-string so the first [1] isn't lost
    entries = re.split(r'(?:^|\n)\s*\[(\d+)\]', ref_section)
    if len(entries) > 2:
        result = []
    ...
    # C3: line fallback with continuation-line merging
...
def _extract_title_guess(raw: str) -> str:
    ...
    # Sentence-boundary split: ". " preceded by >=2 word chars, followed by uppercase.
    # This handles "V. Le" (2 chars) and "Hinton" (6 chars) correctly.
...
# ---------------------------------------------------------------------------
# Helpers — Semantic Scholar with rate limiter + retry on 429  (A2)
# ---------------------------------------------------------------------------
...
def _s2_batch_lookup(ids: list[str]) -> list[dict[str, Any] | None]:
    """Resolve up to 500 paper IDs in a single S2 POST /paper/batch call (A3)."""
    if not ids:
        return []
    _s2_bucket.acquire()
...
def _try_duckduckgo_pdf_search(title: str) -> str | None:
    ...
    try:
        ...
        # DDG wraps result URLs in uddg= redirect parameters
...
def _find_best_pdf_url(paper: dict[str, Any] | None, raw: str = '') -> str | None:
    ...
    Fallback chain  (B1 — DDG promoted to position 4):
      1. S2 openAccessPdf
      2. Direct arXiv PDF (from S2 externalIds)
      3. Direct arXiv PDF (from raw citation text)
      4. DuckDuckGo "title" filetype:pdf search   ← fast, no rate limit
      5. Unpaywall (from DOI)
      6. DOI redirect (content negotiation)

    Source policy:
      S2 batch  — bulk metadata for known DOI/arXiv IDs (one call, rich data)
      S2 search — title fallback (rate-limited ~5 req/s, 429 retries)
      DDG       — fast PDF discovery, no API key; may mis-match on generic
                  titles; scrapes HTML so fragile if DDG changes layout
      Unpaywall — authoritative OA status, DOI-only, moderate latency
      DOI redir — last resort, often hits paywalls
    """
    # 1) S2 openAccessPdf
    if paper:
...
def _resolve_reference(
    ...
    Called from the thread pool for references not already resolved by the
    S2 batch API.  ``pre_ids`` contains identifiers already extracted by
    the caller so we avoid re-parsing.

    Strategy:
      1. arXiv ID → direct PDF URL (fast path, always works).
      2. S2 lookup by DOI or arXiv ID for rich metadata.
      3. S2 title search as fallback.
      4. DDG / Unpaywall / DOI redirect for PDF URL.
      5. Partial result (PDF only) when S2 returned nothing.
    """
