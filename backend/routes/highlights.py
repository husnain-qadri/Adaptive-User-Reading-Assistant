...
from backend.services import document_store, cache
...
def _build_annotated_text(doc: dict[str, Any], client_text: str = '') -> str:
    ...
    When *client_text* is provided (the frontend's PDF.js extraction) we
    use it directly so that the LLM quotes verbatim text the frontend can
    later match via indexOf.  PaperMage section titles are injected as
    lightweight structural markers but the body text comes from the
    client so there is no text-mismatch problem.
    """
